let g:my_vim_wants_to_drop_some_config = 1
let g:myvim_wants_pure_terminal = 1
...
" 各种类型文件的运行函数  "{{{
if 1 || g:my_vim_wants_to_drop_some_config "{{{
    ...
    function! s:RunVimwiki() abort
        ...
        if filereadable( l:newPath . l:curFile . ".html")   "(当前工作目录)有同名的html
            ...
            let options = { 
                        ...
                        \ 'border':[3,3,3,3],
                        \ }
            let popup_id = popup_create('html文本替换成功!', options)
...
if(g:isGUI)
    ...
    set guifont=Cr.DejaVuSansMono.YaHei:h14
else
    "终端vim下的配置
    " 判断变量 myflag 是否存在
...
" -----------------------------------------------------------------------------
if g:my_vim_wants_to_drop_some_config == 0 "{{{
...
" -----------------------------------------------------------------------------
if 0 && g:my_vim_wants_to_drop_some_config==0 "{{{
    if(g:isWindows)
...
function! g:DrawStack()     "画一个示例  func(string x,string y) {...char aaa=1;char bbb=2;}
    call append(line("."),     "     rbp-0 ___old_rbp___ ")
    call append(line(".")+1,   "          |             |")
    call append(line(".")+2,   "          |             |")
    call append(line(".")+3,   "          |             |")
    ...
    call append(line(".")+15,  "  rbp-0x1b|_par x(非基本type) |")
    call append(line(".")+16,  "  rbp-0x1c|_par y(非基本type) |")
endfunction
...
if g:myvim_wants_pure_terminal
    finish
endif
" # < 插件、笔记和一些设置> 
"{{{
...
let g:vimwiki_list = [
            ...
            \ {'path': '~\vimwiki\密钥格式和证书格式-wiki\', 'css_name': 'style.css'},
            \ {'path': '~\vimwiki\签名加密工具帮助文档-wiki\', 'css_name': 'style.css'},
...
if (isGUI)
    if &mousemodel =~? 'popup'
        ...
        anoremenu 1.1  PopUp.@signify查看diff信息.折叠无\ diff\ 信息的区域<tab>(:SignifyFold)
                    \ :SignifyFold<CR>
        ...
        anoremenu 1.5 PopUp.@signify查看diff信息.开启vimdiff模式(右侧出现本文件)<tab>(:SignifyDiff)
                    \ :SignifyDiff<CR>
        anoremenu 1.6 PopUp.@signify查看diff信息.下一处差异<tab>(:normal!\ ]c)
                    \ :normal! ]c <CR>
        anoremenu 1.7 PopUp.@signify查看diff信息.上一处差异<tab>(:normal!\ [c)
                    \ :normal! [c <CR>
